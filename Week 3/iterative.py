...
def iterative_finder(E_array, diff_array, L, K, json_print = False): 
    ...
    #outputting of the data for the last restart in a JSON file for creation of plots
    if json_print:
...
def main():
    ...
    for k in ks:
        #intial roll based on k value sucht that the first starts of an K do not start on the same initial state.
    ...
    #prints the table in a form that is copy ready for latex  
